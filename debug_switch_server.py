...
跟main.py整套循环逻辑完全无关, 只测一件事: 通过CDP执行cp6.disconnect()这个
动作, 在真实游戏里到底会发生什么 —— 有没有真的触发重连、重连后卡在哪个画面
(开局菜单/直接进局/别的中间状态)。上main.py无人值守主循环前务必先跑一遍这个
亲眼确认, 别让没验证过的新动作直接进自动重试逻辑(比照这项目里toggle_map那次
误触发的教训 —— 没验证的假设很容易在实机上跟预期不一样, 而且不容易从日志
倒推出哪一步错了).
...
def main():
    ...
    print("     进了一局/回到开局菜单) —— 没变说明disconnect()没真的执行")
