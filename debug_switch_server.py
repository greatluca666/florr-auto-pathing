...
跟main.py整套循环逻辑完全无关, 只测一件事: 按Esc→点服务器下拉框→点一个服务器
选项这套动作, 在真实游戏里到底会发生什么 —— 有没有真的换到新服务器、全屏有没有
被顶掉、选完之后卡在哪个画面(开局菜单/直接进局/别的中间状态)。上main.py无人
值守主循环前务必先跑一遍这个亲眼确认, 别让没验证过的新动作直接进自动重试逻辑
(比照这项目里toggle_map那次误触发的教训 —— 没验证的UI假设很容易在实机上跟预期
不一样, 而且不容易从日志倒推出哪一步错了).

用法: florr.io开着(真原生全屏), 切到标签页确保它在最前面, 然后跑这个脚本.
5秒倒计时期间去切换窗口.
"""
import time
import pyautogui
...
def main():
    ...
    target = switch_server()
    print(f"   选中的服务器选项坐标: {target}\n")
    ...
    print("  2. 画面确实变了(重新进了一局/回到开局菜单) —— 没变说明下拉框/选项")
    print("     坐标没点中")
