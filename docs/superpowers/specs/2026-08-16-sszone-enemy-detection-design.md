...
- **No `sandstorm.pt` model.** Initially assumed (from the `assets` repo README) to be useful for this; loading it showed a single class `sandstorm` — it detects the sandstorm *weather effect*, not a monster. `desert.pt` (6 classes, confirmed by loading it) is the correct model and is what this spec uses. (The user separately clarified that in-game, "sandstorm" the mob and the weather effect are different things — the weather-effect detector isn't needed here at all.)
